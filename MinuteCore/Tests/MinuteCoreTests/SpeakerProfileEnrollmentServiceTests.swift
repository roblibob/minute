...
struct SpeakerProfileEnrollmentServiceTests {
    ...
    func createProfileFromMeeting_isDeterministic() async throws {
        ...
        #expect(profile.embedding == unitEmbedding(index: 0))
    ...
    @Test
    func updateProfileFromMeeting_overwritesEmbeddingDeterministically() async throws {
        let root = try makeTempDir()
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        #expect(updated.embedding == unitEmbedding(index: 1))
        #expect(updated.updatedAt == clock.now())

        let listed = try await store.listProfiles()
        #expect(listed.count == 1)
        #expect(listed.first?.embedding == unitEmbedding(index: 1))
    }
